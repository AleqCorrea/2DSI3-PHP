...
function ListarAdministrador($cd){
	$sql = 'SELECT * FROM tb_administrador';
	if($id > 0){
		$sql.= ' WHERE cd_administrador = '.$cd;
	}
	$res = $GLOBALS['c']->query($sql);
	return $res;
}
function AtualizarAdministrador($cd,$nome,$email,$senha){
	$sql ='UPDATE tb_administrador SET nm_administrador="'.$nome.'",
		   nm_email = "'.$email.'", cd_senha = "'.$senha'"
		   WHERE cd_administrador = '.$cd;
	$res = $GLOBALS['c']->query($sql);
